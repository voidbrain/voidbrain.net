  ...
  template: `
    <div class="fx-container">
      ...
      <div class="fx-preset-buttons">
        <button (click)="togglePreset1()" class="fx-preset-btn" [class.active]="preset1Active()">
          Ⅰ
        </button>
        <button (click)="togglePreset2()" class="fx-preset-btn" [class.active]="preset2Active()">
          Ⅱ
        </button>
      </div>
    </div>
...
export class FxImageComponent implements AfterViewInit {
  ...
  private getColorMultipliers(): { red: number; green: number; blue: number } {
    switch (`${this.theme()}-${this.color()}`) {
      case 'dark-purple':
        return { red: 0.69, green: 0.3, blue: 1.0 }; // rgba(85, 11, 148, 1);
      case 'dark-orange':
        return { red: 1, green: 0.6, blue: 0.3 };
      case 'dark-green':
        return { red: 0.4, green: 1, blue: 0.5 };
      case 'light-purple':
        return { red: 0.85, green: 0.75, blue: 0.9 }; // #D9BFE6
      case 'light-orange':
        return { red: 0.95, green: 0.85, blue: 0.75 }; // #F2D9BF
      case 'light-green':
        return { red: 0.8, green: 0.95, blue: 0.85 }; // #CCF2D9
      default:
        return { red: 0.69, green: 0.3, blue: 1 };
    }
  }
  ...
  private redraw() {
    ...
    const needsAnimation =
      this.wiggle() || this.crt() || this.vhs() || this.anaglyphMode() || this.applyNoise();
    if (needsAnimation) {
      if (!this.isAnimating) {
  ...
  private renderImage(timeSec: number) {
    ...
    if (!this.anaglyphMode()) {
      ...
      if (this.crt() || this.vhs() || this.wiggle() || this.chroma()) {
        ...
        const rShift = wiggleEnabled
          ? Math.round(Math.sin(t * 2.1) * 2 + Math.cos(t * 0.7) * 1.5)
          : 0;
        const gShift = wiggleEnabled ? Math.round(Math.sin(t * 1.7 + 1) * 2) : 0;
        const bShift = wiggleEnabled
          ? Math.round(Math.sin(t * 1.2 + 2) * 2 + Math.cos(t * 0.5) * 1)
          : 0;
    ...
    for (let i = 0; i < leftData.data.length; i += 4) {
      const gray = Math.round(
        0.299 * leftData.data[i] + 0.587 * leftData.data[i + 1] + 0.114 * leftData.data[i + 2],
      );
      leftData.data[i] = gray;
      leftData.data[i + 1] = gray;
      leftData.data[i + 2] = gray;
    }
    for (let i = 0; i < rightData.data.length; i += 4) {
      const gray = Math.round(
        0.299 * rightData.data[i] + 0.587 * rightData.data[i + 1] + 0.114 * rightData.data[i + 2],
      );
      rightData.data[i] = gray;
      rightData.data[i + 1] = gray;
      rightData.data[i + 2] = gray;
    }
    ...
    if (currentPair.left.name === 'cyan') {
      ...
      for (let i = 0; i < leftData.data.length; i += 4) {
        leftData.data[i] = 0;
        leftData.data[i + 1] = 0; // Block red and green
      }
    } else {
    ...
    if (currentPair.right.name === 'red') {
      ...
      for (let i = 0; i < rightData.data.length; i += 4) {
        rightData.data[i + 1] = 0;
        rightData.data[i + 2] = 0; // Block green/blue channels
      }
      ...
      for (let i = 0; i < rightData.data.length; i += 4) {
        rightData.data[i + 1] = 0;
        rightData.data[i + 2] = 0; // Block green/blue
      }
    }
    ...
    const t = timeSec * this.wiggleSpeed();
    const rShift = wiggleEnabled
      ? Math.round(Math.sin(t * 2.2) * 1.5 + Math.cos(t * 0.6) * 1.2)
      : 0;
  ...
  private applyEffectsToImageData(
    data: Uint8ClampedArray,
    multipliers: { red: number; green: number; blue: number },
    timeSec: number,
  ) {
    ...
    for (let i = 0; i < out.length; i += 4) {
      ...
      if (this.applyNoise()) {
        const n = (Math.random() - 0.5) * 40 + frameNoise;
        r = Math.min(255, Math.max(0, r + n));
        g = Math.min(255, Math.max(0, g + n));
        b = Math.min(255, Math.max(0, b + n));
      }
  ...
  setParallax(val: number) {
    this.anaglyphParallax.set(val);
    this.redraw();
  }
  setAnaglyphColorPair(pairIndex: number) {
    this.anaglyphColorPair.set(pairIndex);
    this.redraw();
  }
  toggleInvert() {
